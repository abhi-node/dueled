...
-- Description: Create all base tables and indexes required for the application
...
CREATE INDEX IF NOT EXISTS idx_player_stats_matches_played ON player_stats(matches_played);
...
CREATE INDEX IF NOT EXISTS idx_match_events_game_time ON match_events(game_time);
...
CREATE INDEX IF NOT EXISTS idx_sessions_expires_at ON player_sessions(expires_at);

CREATE INDEX IF NOT EXISTS idx_password_reset_tokens_token ON password_reset_tokens(token);
CREATE INDEX IF NOT EXISTS idx_password_reset_tokens_player_id ON password_reset_tokens(player_id);
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trigger_update_player_stats_updated_at') THEN
